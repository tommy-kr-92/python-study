...
class ClassificationExcel:
    ...
    def classify(self):
        ...
        for i, row in self.order_list.iterrows():  # iterrow()는 두가지를 가져와야 함
            ...
            if partner_name != '':
                ...
                # print(df_filtered)
                df_filtered.to_excel(f'{self.path}/[패스트몰] {partner_name}.xlsx', engine='openpyxl', index=False)
            else:
    ...
    def set_form(self, file_name):
        ...
        for row in ws.iter_rows(min_row=4):
            for cell in row:
                cell.alignment = Alignment(wrap_text=True, horizontal='center', vertical='center', shrink_to_fit=True)   # shrink_to_fit - 칸이 가득 찾을 때 글자 작게
