...
class EmailSender:
    ...
    def send_email(self, html_msg, from_addr, to_addr, receiver_name, subject, attachment, cc_addr=''):
        ...
        with smtplib.SMTP(self.smtp_server, 587) as smtp:
            msg =  MIMEMultipart('alternative')     # 네이버 일 때만 필요 - 시작
            ...
            if attachment:
                # 첨부파일 읽기
                with open(f'{self.path}{attachment}.xlsx', 'rb') as f:
...
if __name__ == '__main__':
    es = EmailSender('jyhuh8775@naver.com', os.getenv('MY_NAVER_PASSWORD'), '김미령', path='data/')
