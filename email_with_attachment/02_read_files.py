...
def make_email_list(data_path, partners_filename, title, target_filename='이메일발송목록.xlsx'):
    ...
    for filename in filenames:
        ...
        if email_cc == 'nan':
            email_cc = ''
        info = {'담당자메일': email1, '참조': email_cc, '컨텍담당자': partners_filename, '제목': title, '첨부파일명': filename}
        r.append(info)
...
if __name__ == '__main__':
    make_email_list('data/', '파트너목록.xlsx', '[패스트몰] 금일 발주 목록 입니다. 확인 부탁드립니다.', 'email_list.xlsx')